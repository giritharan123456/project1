...
import { HiArrowLeft, HiShieldCheck, HiCheckCircle, HiXCircle } from 'react-icons/hi';
...
export default function OTPVerificationPage() {
  ...
  const handleVerify = async () => {
    ...
    // Accept the code shown on this page (SMS/email simulation)
    setStatus('success');
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-primary-50 dark:from-slate-900 dark:to-slate-800 px-4 py-12">
        <motion.div initial="hidden" animate="visible" variants={containerVariants} className="w-full max-w-md">
          <motion.div variants={itemVariants} className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-gray-200 dark:border-slate-700 p-8 text-center">
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 200, damping: 15 }} className="w-20 h-20 bg-emerald-100 dark:bg-emerald-900/30 rounded-full flex items-center justify-center mx-auto mb-4">
            ...
            <p className="text-gray-600 dark:text-slate-400 mb-6">Your email has been verified successfully. You can now proceed.</p>
            <Button onClick={() => navigate('/auth/2fa', { state: { role } })} fullWidth size="lg">Continue to 2FA</Button>
          </motion.div>
        </motion.div>
      </div>
    );
  ...
  if (status === 'failed') {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-primary-50 dark:from-slate-900 dark:to-slate-800 px-4 py-12">
        <motion.div initial="hidden" animate="visible" variants={containerVariants} className="w-full max-w-md">
          <motion.div variants={itemVariants} className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-gray-200 dark:border-slate-700 p-8 text-center">
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 200, damping: 15 }} className="w-20 h-20 bg-red-100 dark:bg-red-900/30 rounded-full flex items-center justify-center mx-auto mb-4">
            ...
            <p className="text-gray-600 dark:text-slate-400 mb-6">The code you entered is incorrect. Please try again or request a new code.</p>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-primary-50 dark:from-slate-900 dark:to-slate-800 px-4 py-12">
      <Helmet>
        <title>OTP Verification - AdzConnect</title>
        <meta name="description" content="Verify your identity with a one-time passcode sent to your email for AdzConnect." />
      </Helmet>
      <motion.div
        initial="hidden"
        animate="visible"
      ...
      >
        <motion.div variants={itemVariants} className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 dark:text-slate-100">Verify your identity</h1>
          <p className="mt-2 text-gray-600 dark:text-slate-400">Enter the verification code sent to your email</p>
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-gray-200 dark:border-slate-700 p-8">
          ...
          <Button onClick={handleVerify} fullWidth loading={isLoading} size="lg">
            Verify Code
          </Button>

          <div className="text-center mt-6">
            {canResend ? (
              <button onClick={handleResend} className="text-sm font-medium text-primary-600 dark:text-primary-400 hover:underline">
                Resend code
              </button>
            ) : (
              <p className="text-sm text-gray-500 dark:text-slate-400">
                Resend code in <span className="font-medium text-gray-900 dark:text-slate-200">{timer}s</span>
              </p>
            )}
          </div>
        </motion.div>
        ...
        <motion.p variants={itemVariants} className="text-center mt-6 text-sm">
          <Link to="/auth/forgot-password" className="inline-flex items-center gap-2 font-medium text-primary-600 dark:text-primary-400 hover:underline">
