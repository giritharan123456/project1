...
export default function RoleTwoFactorContent({ role }) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-6 text-sm text-gray-600 dark:text-slate-400 space-y-2">
        ...
        <Link to={`/auth/${role}/login`} className="inline-flex items-center gap-1 text-primary-600 dark:text-primary-400 hover:underline font-medium">
...
export function RoleTwoFactorPage({ role }) {
  return (
    <RoleAuthWrapper role={role} helmetTitle={`2FA - ${role.charAt(0).toUpperCase() + role.slice(1)} - AdzConnect`}>
      <RoleTwoFactorContent role={role} />
    </RoleAuthWrapper>
  );
}
